	...
	int sofar=0;
	 ...
	 /* Unlock the Flash to enable the flash control register access *************/
	   ...
	   * (area defined by FLASH_USER_START_ADDR and FLASH_USER_END_ADDR) ***********/
	   ...
	   while (sofar<numberofwords && StartSectorAddress < (APPLICATION_END_ADDRESS - 4*FLASHWORD))
	   {
...
// buf has to be at least the size of 4*size
...
FlashProtectionType FlashGetWriteProtectionStatus() {
    FlashProtectionType ProtectedPAGE = FLASHIF_PROTECTION_NONE;
    ...
    if(OptionsBytesStruct2.PCROPEndAddr > OptionsBytesStruct2.PCROPStartAddr) {
        /* check if user area are included inside this range */
        if(OptionsBytesStruct2.PCROPStartAddr > APPLICATION_ADDRESS) {
            ProtectedPAGE |= FLASHIF_PROTECTION_PCROPENABLED;
        }
    }

    /* check WRP */
    if(OptionsBytesStruct1.WRPState == OB_WRPSTATE_ENABLE || OptionsBytesStruct2.WRPState == OB_WRPSTATE_ENABLE) {
        ProtectedPAGE |= FLASHIF_PROTECTION_WRPENABLED;
    }

    if(OptionsBytesStruct1.RDPLevel != OB_RDP_LEVEL_0 || OptionsBytesStruct2.RDPLevel != OB_RDP_LEVEL_0) {
