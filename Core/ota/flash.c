...
static char* s_ota_addr; // Current address to write to
static size_t s_ota_size; // Firmware size to flash. In-progress indicator
static uint32_t s_ota_crc32; // Firmware checksum
...
bool ota_flash_begin(size_t new_fw_size, FlashItf* flashItf){
    ...
    if(s_ota_size){
        ...
        s_ota_crc32 = 0;
        s_ota_addr = (char*)flashItf->start + half;
        if(new_fw_size < half){
            ok = true;
            s_ota_size = new_fw_size;
...
bool ota_flash_write(const void* buf, size_t len, FlashItf* flashItf) {
    ...
    if(s_ota_size == 0){
        ...
        if(len_aligned_down){
            ok = flashItf->write_fn(s_ota_addr, buf, len_aligned_down);
        }
        if(len_aligned_down < len){
            ...
            ok = flashItf->write_fn(s_ota_addr + len_aligned_down, tmp, sizeof(tmp));
        }
        s_ota_crc32 = crc32(s_ota_crc32, (char*)buf, len); // Update CRC
        s_ota_addr += len;
    }
    return ok;
}
...
bool ota_flash_end(FlashItf* flashItf) {
    ...
    bool ok = false;
    if(s_ota_size){
        size_t size = (size_t)(s_ota_addr - base);
        uint32_t crc32_ = crc32(0, base, s_ota_size);
        if(size == s_ota_size && crc32_ == s_ota_crc32){
            ok = true;
        }
        s_ota_size = 0;
        if(ok){
